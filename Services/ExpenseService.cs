...
namespace ExpensesInfo.Services
{
    public class ExpenseService : IExpenseService
    {
        ...
        {
            var query = _context.Expenses.Include(e => e.ExpenseType).AsQueryable();
        ...
        {
            var existing = await _context.Expenses.SingleOrDefaultAsync(e => e.Id == expense.Id);
        ...
        {
            var existing = await _context.Expenses.SingleOrDefaultAsync(e => e.Id == id);
        ...
        public async Task<decimal> GetTotalAsync(int? typeId)
        {
            ...
            if (typeId.HasValue)
            {
                query = query.Where(equals => equals.ExpenseTypeId == typeId.Value);
            }
